"""SDLC tool catalog — shared implementations used by deep subagents."""
...
from src.fixtures import load_sdlc
...
def list_failed_pipelines(service: str = "payments-service", limit: int = 5) -> str:
    """List recent failed CI/CD pipeline runs for a service (fixture data)."""
...
def list_pending_approvals(service: str = "payments-service") -> str:
    """List pipeline approval gates still waiting (fixture data)."""
...
def get_pipeline_run(run_id: int) -> str:
    """Get details for a specific pipeline run id (fixture data)."""
...
def list_deployments(service: str = "payments-service") -> str:
    """List recent deployments across environments for the release train."""
    _ = service
...
# ---------------------------------------------------------------------------
# Research tools (web search with fixture fallback)
# ---------------------------------------------------------------------------
...
def web_search(query: str, max_results: int = 5) -> str:
    """Search the web for outages, CVEs, or release-risk context. Uses Exa/Tavily if keyed, else fixtures."""
...
def fetch_url_summary(url: str) -> str:
    """Summarize a URL for release risk context (lightweight stub + fixture awareness)."""
    ...
    # Best-effort fetch of title-ish content without HTML parsing deps
    try:
        import urllib.request

        req = urllib.request.Request(url, headers={"User-Agent": "deepagent-poc/1.0"})
...
def build_insight_chart(
    ...
    """Build a release-risk chart PNG from SLO burn fixtures; returns file path + ASCII summary."""
    row = load_sdlc()["slo_burn"].get(service)
    if not row:
    ...
    # ASCII sparkline-ish summary
    max_e = max(errors) or 1
    ascii_bars = " | ".join(f"{d}:{'#' * max(1, int(e / max_e * 8))}" for d, e in zip(days, errors))
...
def compute_release_risk_score(service: str = "payments-service") -> str:
    """Compute a simple 0-100 release risk score from fixtures (higher = riskier)."""
    data = load_sdlc()
    ...
    return _j(
        {
            ...
            "recommendation": "hold" if score >= 60 else "proceed_with_caution" if score >= 40 else "go",
        }
    )
...
def all_tools() -> list[BaseTool]:
    """Full flat MCP-style tool list (~18)."""
    return [*WORKFLOW_TOOLS, *RESEARCH_TOOLS, *INSIGHTS_TOOLS]
